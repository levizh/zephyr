 ...
 */
static int
	gpio_hc32_cfg2ll_output_status(gpio_flags_t flags, uint16_t *out_status)
{
				...
				gpio_flags_t flags)
{
	...
	if ((flags & GPIO_OUTPUT) != 0U) {
		...
		if (0 != gpio_hc32_cfg2ll_output_type(flags,
				&stc_gpio_init.u16PinOutputType)) {
			return -ENOTSUP;
		}
		...
		if (0 != gpio_hc32_cfg2ll_output_status(flags,
				&stc_gpio_init.u16PinState)) {
			/* Don't support logical set */
			return -ENOTSUP;
		}
