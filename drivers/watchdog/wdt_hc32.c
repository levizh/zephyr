...
#define WDT_PROP_ARRAY_BY_IDX(index, prop, offset)          \
        DT_XHSC_HC32_WATCHDOG_##index##_##prop##_##offset
#define WDT_PROP_SINGLE_BY_IDX(index, prop)                 \
        DT_XHSC_HC32_WATCHDOG_##index##_##prop

/* device name */
#define WDT_DEVICE_NAME                 wdt_hc32
 ...
 */
static uint32_t wdt_hc32_get_timeout(struct device *dev,
 ...
 */
static int wdt_hc32_find_index(struct device *dev, uint32_t timeout)
{
	int matchIdx;
	uint32_t curVal;
...
static int wdt_hc32_setup(struct device *dev, uint8_t options)
{
...
static int wdt_hc32_disable(struct device *dev)
{
...
static int wdt_hc32_install_timeout(struct device *dev,
...
static int wdt_hc32_feed(struct device *dev, int channel_id)
{
	ARG_UNUSED(dev);
...
static int wdt_hc32_init(struct device *dev)
{
	ARG_UNUSED(dev);
	return 0;
}
...
DEVICE_AND_API_INIT(WDT_DEVICE_NAME, WDT_PROP_SINGLE_BY_IDX(0, LABEL),
		    wdt_hc32_init, &wdt_hc32_dev_data, NULL,
