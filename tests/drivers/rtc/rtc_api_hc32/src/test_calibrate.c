...
#define RTC_TEST_CAL_RANGE_LIMIT (276000)
#define RTC_TEST_CAL_RANGE_STEP	 (960) /* 0.96ppm */
...
static int test_set_get_calibration(int32_t calibrate_set)
{
	...
	diff = calibrate_get - calibrate_set;
	/* Print comparison between set and get values */
	printk("Calibrate (set,get,diff): %i, %i, %i\n", calibrate_set, calibrate_get, diff);

	zassert_true((fabs(diff) < RTC_TEST_CAL_RANGE_STEP), "calibrate diff is too large!!");
